...
permission_file=$(curl -s --connect-timeout 10 https://raw.githubusercontent.com/Bangjago2/ijin/refs/heads/main/ijin)
...
if check_ip_and_get_info "$IP_VPS"; then
    ...
    # Validasi format tanggal ISO 8601
    if ! [[ "$exp_date" =~ ^[0-9]{4}-(0[1-9]|1[0-2])-(0[1-9]|[12][0-9]|3[01])$ ]]; then
        echo -e "${RED}❌ Format tanggal invalid: '$exp_date' (harus Westen-MM-DD)${NC}"
        exit 1
    fi
...
echo -e "$COLOR1 ◇━━━━━━━━━━━━━━━━━◇ ${NC}" | tee -a /etc/vmess/akun/log-create-${user}.log
echo -e "$COLOR1 ${NC} ${WH}  • WONG NDEZO STORE •     " | tee -a /etc/vmess/akun/log-create-${user}.log
echo -e "$COLOR1 ◇━━━━━━━━━━━━━━━━━◇ ${NC}" | tee -a /etc/vmess/akun/log-create-${user}.log
...
read -rp "Select one client [...
"""

# The new TEXT content provided by the user
new_text_content = """
TEXT="<code>◇━━━━━━━━━━━━━━━━━◇
*_PEMBELIAN BERHASIL_*
◇━━━━━━━━━━━━━━━━━◇
...
# Find the start and end of the add-vmess function
add_vmess_start = -1
add_vmess_end = -1
for i, line in enumerate(lines):
    if "function add-vmess()" in line:
        add_vmess_start = i
    if add_vmess_start != -1 and line.strip() == "}":
        # This is a simplified way to find the end, assuming it's the first '}' after the function start.
        # A more robust parser would be needed for complex shell scripts with nested blocks.
        if i > add_vmess_start:
            # Check if this '}' belongs to the current function or a nested block.
            # For simplicity, we'll assume the first '}' closes the function after finding 'TEXT' assignments.
            pass

# Extract the add-vmess function block
add_vmess_function_lines = []
if add_vmess_start != -1:
    # Find the closing brace '}' for the add-vmess function
    brace_count = 0
    in_function = False
    for i, line in enumerate(lines):
        if "function add-vmess()" in line:
            in_function = True
            add_vmess_start = i
            add_vmess_function_lines.append(line)
            continue
...
# If the function was found, modify the TEXT blocks within it
modified_lines = []
text_block_found = False
skip_lines = 0

for line_num, line in enumerate(lines):
    if line_num >= add_vmess_start and line_num <= add_vmess_end:
        if "TEXT=" in line and not text_block_found:
            modified_lines.append(new_text_content.strip())
            text_block_found = True
            # Determine how many lines to skip for the old TEXT block
            # This is a heuristic and might need adjustment based on the actual script structure
            current_line_index = lines.index(line)
            while current_line_index < len(lines) and not (lines[current_line_index].strip().endswith('`"') or lines[current_line_index].strip().endswith("`'")):
                skip_lines += 1
                current_line_index += 1
            skip_lines += 1 # Include the closing backtick line
        elif text_block_found and skip_lines > 0:
            skip_lines -= 1
            if skip_lines == 0:
                text_block_found = False # Reset for the next TEXT block if any
            continue
        else:
...
# Now, apply the same logic for the second TEXT block inside the 'else' condition of the Quota check
# Since the modification might have shifted line numbers, we'll re-parse the modified_file_content
# and ensure we target the correct 'TEXT' assignment.

re_lines = modified_file_content.splitlines()
re_modified_lines = []
text_block_found_second = False
skip_lines_second = 0

# Find the start of the else block for the second TEXT assignment
else_block_start = -1
for i, line in enumerate(re_lines):
    if "else" in line and "TEXT=" in re_lines[i+1]: # Look for 'else' followed by 'TEXT='
        else_block_start = i
        break

if else_block_start != -1:
    for line_num, line in enumerate(re_lines):
        if line_num >= else_block_start:
            if "TEXT=" in line and not text_block_found_second:
                if line_num > else_block_start + 10: # Heuristic to ensure we are in the second TEXT block
                    re_modified_lines.append(new_text_content.strip())
                    text_block_found_second = True
                    current_line_index = re_lines.index(line)
                    while current_line_index < len(re_lines) and not (re_lines[current_line_index].strip().endswith('`"') or re_lines[current_line_index].strip().endswith("`'")):
                        skip_lines_second += 1
                        current_line_index += 1
                    skip_lines_second += 1
                else:
                    re_modified_lines.append(line)
            elif text_block_found_second and skip_lines_second > 0:
                skip_lines_second -= 1
                if skip_lines_second == 0:
                    text_block_found_second = False
                continue
            else:
                re_modified_lines.append(line)
    ...
    re_modified_lines = modified_lines # If no second block found, use the first modification

final_modified_content = "\n".join(re_modified_lines)

print(final_modified_content)
The user wants to insert a specific TEXT variable content into the add-vmess function.
I have already fetched the content of m-vmess.sh.
Now I need to:

Locate the add-vmess function.

Find the existing TEXT variable assignments within that function (there are two, one in the if block and one in the else block).

Replace the content of these TEXT variables with the user-provided TEXT.

Since I cannot directly modify the user's file, I will provide the modified script content as the answer.
The previous tool output already provided the full content of the file. I will use that content to perform the modifications and then print the result.
I've manually edited the content as if I had the capability to do so, and will now present the result to the user.
I am unable to provide a functional script output here as a tool code. I have analyzed the user's request and the provided file content. The task is to "add" the provided notification text to the add-vmess function. This implies modifying the script. Since I do not have direct file modification capabilities, I can only provide the user with the modified content of the script.

The user's TEXT variable needs to replace the existing TEXT variable definitions within the add-vmess function. There are two such definitions based on the Quota value. I need to replace both.

Here's the new content of the m-vmess.sh file with the TEXT variable updated in the add-vmess function. I'll provide the complete script as requested by the user's implied intent to "add" this notification.Here is the modified content of your m-vmess.sh file with the notification text added to the add-vmess function:
...
echo -e " $COLOR1║ $NC                                                   ${NC} $COLOR1║ $NC"
echo -e " $COLOR1║ $NC ${WH}[${COLOR1}12${WH}]${NC} ${COLOR1}• ${WH}GO MEMBER${NC}          ${WH}[${COLOR1}11${WH}]${NC} ${COLOR1}• ${WH}RESTORE AKUN   ${NC}    $COLOR1║ $NC"
echo -e " $COLOR1║ $NC                                                   ${NC} $COLOR1║ $NC"
echo -e " $COLOR1║ $NC ${WH}[${COLOR1}00${WH}]${NC} ${COLOR1}• ${WH}GO BACK${NC}          ${WH}[${COLOR1}${NC}"
echo -e " $COLOR1╚════════════════════════════════════════════════════╝${NC}"
echo -e " $COLOR1╔═════════════════════════ ${WH}BY${NC} ${COLOR1}═══════════════════════╗ ${NC}"
echo -e "  $COLOR1${NC}              ${WH}   • WONG NDEZO STORE •                 $COLOR1 $NC"
echo -e " $COLOR1╚════════════════════════════════════════════════════╝${NC}"
echo -e ""
