...
if check_ip_and_get_info "$IP_VPS"; then
    ...
    echo -e "➥ Hubungi admin ${CYAN}「 ✦ @Gxkuwi ✦ 」${NC}"
    exit 1
fi
...
<b>Limit IP       :</b> <code>${iplim} IP</code>
<code>◇━━━━━━━━━━━━━━━━━◇</code>
<b>ISP            :</b> $ISP
<b>CITY           :</b> $CITY
...
<b>BadVPN UDP     :</b> 7100, 7300, 7300
<code>◇━━━━━━━━━━━━━━━━━◇</code>
<b>SSH UDP        :</b> <code>$domen:1-65535@$Login:$Pass</code>
...
<code>◇━━━━━━━━━━━━━━━━━◇</code>
<b>Payload ENHACED:</b>
<code>GET / HTTP/1.1[crlf]Host: bug.com[crlf][crlf]PATCH / HTTP/1.1[crlf]Host: [host][crlf]Upgrade: websocket[crlf][crlf][split]HTTP/ 69[crlf][crlf]</code>
<code>◇━━━━━━━━━━━━━━━━━◇</code>
<b>Payload WS/WSS:</b>
...
<code>◇━━━━━━━━━━━━━━━━━◇</code>
<b>OpenVPN File   :</b> http://$domen:89/ssl.ovpn
<b>Save Link      :</b> http://$domen:89/ssh-$Login.txt
<code>◇━━━━━━━━━━━━━━━━━◇</code>
<b>Masa Aktif     :</b> $masaaktif Hari
<b>Expired On     :</b> $exp
<code>◇━━━━━━━━━━━━━━━━━◇</code>
"

# =================== PESAN 2: LOG TRANSAKSI =====================
CHATID2="$CHATID"  # Atau isi ID lain jika mau kirim ke grup berbeda
...
echo -e "$COLOR1 ◇━━━━━━━━━━━━━━━━━◇ ${NC}" | tee -a /etc/xray/sshx/akun/log-create-${Login}.log
echo -e "$COLOR1 ${NC}    ${WH}• WONG NDEZO STORE •${NC}                 $COLOR1 $NC" | tee -a /etc/xray/sshx/akun/log-create-${Login}.log
echo -e "$COLOR1 ◇━━━━━━━━━━━━━━━━━◇ ${NC}" | tee -a /etc/xray/sshx/akun/log-create-${Login}.log
...
# Kirim juga via /etc/tele jika ada
if [ -e /etc/tele ]; then
  echo "$TEXT" > /etc/notiftele
  bash /etc/tele
fi

# === Notifikasi Kedua (Log Transaksi) ===
CHATID2="$CHATID"   # Kalau pakai chat ID yang sama, atau ganti ke ID berbeda
URL2="https://api.telegram.org/bot$KEY/sendMessage"
...
<b>ISP    :</b> <code>${CITY}</code>
<b>DATE   :</b> <code>${TIME2} WIB</code>
<b>DETAIL :</b> <code>Trx SSH Renew</code>
<b>USER   :</b> <code>${user2}xxx</code>
...
function hapus(){
  ...
  until [[ ${CLIENT_NUMBER} -ge 1 && ${CLIENT_NUMBER} -le ${NUMBER_OF_CLIENTS} ]]; do
    if [[ ${NUMBER_OF_CLIENTS} == '1' ]]; then
      read -rp "Select one client [1]: " CLIENT_NUMBER
      CLIENT_NUMBER=${CLIENT_NUMBER:-1}  # default ke 1 kalau kosong
    else
      read -rp "Select one client [1-${NUMBER_OF_CLIENTS}]: " CLIENT_NUMBER
      if [[ ${CLIENT_NUMBER} == '0' ]]; then
        sshws
  ...
  # Hapus baris user di config
  sed -i "/^### $Pengguna $Days $Pass/d" /etc/xray/ssh

  # Hapus file terkait user
  rm -f /home/vps/public_html/ssh-$Pengguna.txt
  rm -f /etc/xray/sshx/${Pengguna}IP
  rm -f /etc/xray/sshx/${Pengguna}login

  # Hapus user di sistem
  ...
  # Buat pesan notifikasi multiline dengan heredoc
  read -r -d '' TEXT <<EOF
<code>◇━━━━━━━━━━━━━━◇</code>
<b>  DELETE SSH OVPN</b>
<code>◇━━━━━━━━━━━━━━◇</code>
...
function cek(){
    ...
    echo -e "       \033[1;37m>> DROPBEAR ACTIVE SESSIONS <<\033[0m"
    echo -e "\033[1;36m└────────────────────────────────────────────┘\033[0m"
    echo -e "\033[1;36mPID     | Username     | IP Address\033[0m"
    echo -e "\033[1;36m──────────────────────────────────────────────\033[0m"

    cat "$LOG" | grep -i "dropbear.*Password auth succeeded" > /tmp/dropbear-login.txt
    for pid in $(pgrep dropbear); do
        line=$(grep "dropbear\[$pid\]" /tmp/dropbear-login.txt)
        if [ ! -z "$line" ]; then
    ...
    echo ""
    echo -e "\033[1;36m┌────────────────────────────────────────────┐\033[0m"
    echo -e "       \033[1;37m>> SSHD ACTIVE SESSIONS <<\033[0m"
    echo -e "\033[1;36m└────────────────────────────────────────────┘\033[0m"
    echo -e "\033[1;36mPID     | Username     | IP Address\033[0m"
    echo -e "\033[1;36m──────────────────────────────────────────────\033[0m"

    cat "$LOG" | grep -i "sshd.*Accepted password" > /tmp/sshd-login.txt
    for pid in $(ps aux | grep sshd | grep '\[priv\]' | awk '{print $2}'); do
        line=$(grep "sshd\[$pid\]" /tmp/sshd-login.txt)
        if [ ! -z "$line" ]; then
    ...
    if [ -f "/etc/openvpn/server/openvpn-tcp.log" ]; then
        echo ""
        echo -e "\033[1;36m┌────────────────────────────────────────────┐\033[0m"
        echo -e "       \033[1;37m>> OPENVPN TCP ACTIVE SESSIONS <<\033[0m"
        echo -e "\033[1;36m└────────────────────────────────────────────┘\033[0m"
        echo -e "\033[1;36mUsername        | IP Address      | Connected Since\033[0m"
        echo -e "\033[1;36m──────────────────────────────────────────────\033[0m"
    ...
    if [ -f "/etc/openvpn/server/openvpn-udp.log" ]; then
        echo ""
        echo -e "\033[1;36m┌────────────────────────────────────────────┐\033[0m"
        echo -e "       \033[1;37m>> OPENVPN UDP ACTIVE SESSIONS <<\033[0m"
        echo -e "\033[1;36m└────────────────────────────────────────────┘\033[0m"
        echo -e "\033[1;36mUsername        | IP Address      | Connected Since\033[0m"
        echo -e "\033[1;36m──────────────────────────────────────────────\033[0m"
...
function limitssh(){
  ...
  # Buat pesan multiline dengan heredoc supaya aman
  read -r -d '' TEXT <<EOF
<code>◇━━━━━━━━━━━━━━◇</code>
<b>  SSH IP LIMIT</b>
<code>◇━━━━━━━━━━━━━━◇</code>
