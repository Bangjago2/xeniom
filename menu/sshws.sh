...
# Deteksi log file yang tersedia
...
# Ambil log login sukses Dropbear
grep -i "dropbear.*password auth succeeded" "$LOG" > /tmp/dropbear-login.txt
...
# Ambil log login sukses SSHD
grep -i "sshd.*Accepted" "$LOG" > /tmp/sshd-login.txt

for pid in $(ps aux | grep '[s]shd' | grep '\[priv\]' | awk '{print $2}'); do
...
# Fallback info: tampilkan sesi dari 'who' jika tidak ada yang terdeteksi
echo ""
echo -e "\033[1;36m┌────────────────────────────────────────────┐\033[0m"
echo -e "       \033[1;37m>> WHO (Fallback Sesi Login) <<\033[0m"
echo -e "\033[1;36m└────────────────────────────────────────────┘\033[0m"
who
