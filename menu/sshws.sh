...
# New notification text to be added
TEXT2="<code>◇━━━━━━━━━━━━━━━━━◇
...
if [ -e "/var/log/auth.log" ]; then
    ...
    echo -e "\033[0;31mError: Auth log file not found. Exiting.\033[0m"
    exit 1
fi
...
# Function to display VPN header
display_vpn_header() {
    echo -e "\033[1;36m┌──────────────────────────────────────────┐\033[0m"
...
echo -e "\n\033[1;33m--- Active Dropbear Sessions ---\033[0m"
display_header

# Get PIDs of active dropbear processes
# Using 'pgrep -f' is more robust than 'ps aux | grep' for finding processes
# and 'awk' to get the PID.
dropbear_pids=$(pgrep -f dropbear)

if [ -n "$dropbear_pids" ]; then
    # Create a temporary file for successful Dropbear logins
    grep -i "dropbear" "$LOG" | grep -i "Password auth succeeded" > /tmp/login-db.txt

    for PID in $dropbear_pids; do
        # Use 'grep -q' to check for existence without printing, then extract data
        if grep -q "dropbear\[$PID\]" /tmp/login-db.txt; then
            # Extract user and IP directly from the log line associated with the PID
            user=$(grep "dropbear\[$PID\]" /tmp/login-db.txt | awk '{print $10}' | tail -n 1) # tail -n 1 to get the latest
            ip=$(grep "dropbear\[$PID\]" /tmp/login-db.txt | awk '{print $12}' | tail -n 1) # tail -n 1 to get the latest
            echo "$PID - $user - $ip"
        fi
    done
    rm -f /tmp/login-db.txt # Clean up temporary file
else
    echo "    No active Dropbear sessions found."
fi
echo -e "\033[1;36m└──────────────────────────────────────────┘\033[0m"

# --- Monitor SSH (sshd) Sessions ---
echo -e "\n\033[1;33m--- Active SSHD Sessions ---\033[0m"
display_header

# Get PIDs of active sshd processes (specifically "priv" processes for established sessions)
sshd_pids=$(ps aux | grep "\[priv\]" | awk '{print $2}')

if [ -n "$sshd_pids" ]; then
    # Create a temporary file for accepted SSH logins
    grep -i "sshd" "$LOG" | grep -i "Accepted password for" > /tmp/login-ssh.txt

    for PID in $sshd_pids; do
        if grep -q "sshd\[$PID\]" /tmp/login-ssh.txt; then
            # Extract user and IP directly from the log line associated with the PID
            user=$(grep "sshd\[$PID\]" /tmp/login-ssh.txt | awk '{print $9}' | tail -n 1) # tail -n 1 to get the latest
            ip=$(grep "sshd\[$PID\]" /tmp/login-ssh.txt | awk '{print $11}' | tail -n 1) # tail -n 1 to get the latest
            echo "$PID - $user - $ip"
        fi
    done
    rm -f /tmp/login-ssh.txt # Clean up temporary file
else
    echo "    No active SSHD sessions found."
fi
echo -e "\033[1;36m└──────────────────────────────────────────┘\033[0m"


# --- Monitor OpenVPN TCP Sessions ---
if [ -f "/etc/openvpn/server/openvpn-tcp.log" ]; then
    echo -e "\n\033[1;33m--- Active OpenVPN TCP Sessions ---\033[0m"
    display_vpn_header
